...
| 2026-06-17 | **服务端代码提交后立即部署** | Git push 后自动触发部署到腾讯云，保持线上与代码仓库一致 |
| 2026-06-25 | **每次开发完成后自动运行 bash run_tests.sh** | 沙箱中自动执行 Go 后端 58 个测试，验证无回归，测试报告自动提交到 Git |
| 2026-06-25 | **所有新功能提交推送到 v3.0-dev 分支** | 禁止推送到 main；HTTPS+Token 方式确保沙箱重建后无需重新配置 SSH |
| 2026-06-25 | **测试报告自动沉淀到 docs/tests/ 目录** | 每次 run_tests.sh 生成带时间戳的报告，提交到两个仓库 |
