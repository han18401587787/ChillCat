    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: AppSpacing.lg) {
                RoundedRectangle(cornerRadius: AppRadius.md).fill(Color(hex:"D4C8E8").opacity(0.3)).frame(height: 180).overlay(
                ...
                Button(action: { withAnimation { progress = min(1, progress + 0.25) } }) {
                    Text(progress >= 1 ? "已完成" : "标记进展").fontWeight(.medium).foregroundColor(.white).frame(maxWidth:.infinity).padding(.vertical,14).background(progress>=1 ? Color(hex:"66BB6A") : Color(hex:"5A7A8A")).cornerRadius(AppRadius.md)
                }
            }.padding()
