    ...
    var body: some View {
        ScrollView {
            VStack(alignment:.leading,spacing:theme.spacingMD) {
                ...
                Text("更新日期：2026年6月").font(.system(size:13)).foregroundColor(theme.textMuted)
        ...
        }.background(theme.background).navigationTitle("隐私政策")
    }
    func section(_ title:String,_ body:String) -> some View {
        VStack(alignment:.leading,spacing:8){
            Text(title).font(.system(size:16,weight:.semibold))
            Text(body).font(.system(size:14)).foregroundColor(theme.textSecondary).lineSpacing(6)
        }.padding(.bottom,8)
    }
}
